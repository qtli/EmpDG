...
class Generator(nn.Module):
    ...
    def forward(self, x, pred_emotion=None, emotion_context=None, attn_dist=None, enc_batch_extend_vocab=None,
        ...
        if emotion_context is not None:  # considering emotions of target (future)
...
class EmpDG_woG(nn.Module):
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        enc_emo_batch = batch['context_emotion_batch']

        oovs = batch["oovs"]
        max_oov_length = len(sorted(oovs, key=lambda i: len(i), reverse=True)[0])

        dec_batch = batch["target_batch"]
        dec_fedb_batch = batch["target_feedback_batch"]
        dec_emo_batch = batch["target_emotion_batch"]
        ...
        mask_emotion = enc_batch.data.eq(config.PAD_idx).unsqueeze(1)
