...
class EmpDG_woD(nn.Module):
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        # emo_encoder_outputs = self.semantic_und(emo_emb, mask_emotion)  # C_e  (bsz, emo_w_len, emb_dim)
    ...
    def decoder_greedy(self, batch, max_dec_step=30):
        ...
        if config.adver_train:
            ys = torch.ones(config.batch_size, 1).fill_(config.SOS_idx).long()
        else:
