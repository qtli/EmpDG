    ...
    def forward(self, inputs, encoder_output, pred_emotion=None, emotion_contexts=None, mask=None):
        ...
        if(self.universal):
            if(config.act):
                ...
                for l in range(self.num_layers):
                    x += self.position_signal[:, l, :].unsqueeze(1).repeat(1,inputs.shape[1],1).type_as(inputs.data)
                    x, _, attn_dist, _ = self.dec((x, encoder_output, pred_emotion, emotion_contexts, [], (mask_src,dec_mask)))
                y = self.layer_norm(x)
        else:
            # Add timing signal
            x += self.timing_signal[:, :inputs.shape[1], :].type_as(inputs.data)
...
class Generator(nn.Module):
    ...
    def forward(self, x, pred_emotion=None, emotion_context=None, attn_dist=None, enc_batch_extend_vocab=None, max_oov_length=None, temp=1, beam_search=False, attn_dist_db=None):
...
class MK_Dec(nn.Module):
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        ## emotional signal distilling
        if concept_input.size()[0] != 0:
            concept_context_vad = torch.cat((batch["context_vad"], batch["concept_vad_batch"]), dim=1)  # (bsz, context_len+concept_len)
        else:
        ...
        logit = self.generator(pre_logit, None, None, attn_dist, enc_ext_batch if config.pointer_gen else None, max_oov_length, attn_dist_db=None)
    ...
    def decoder_greedy(self, batch, max_dec_step=30):
        ...
        for i in range(max_dec_step+1):
            if config.project:
                out, attn_dist = self.decoder(self.embedding_proj_in(ys_emb), self.embedding_proj_in(encoder_outputs), (mask_src,mask_trg))
            else:
