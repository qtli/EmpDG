...
def wordlist2oov(src_words):
    ...
    for w in src_words:
        if w in config.w2id:
            ...
            ids.append(config.vocab_size + oov_num)  # This is e.g. 50000 for the first article OOV, 50001 for the second...
    return ids, oovs


def rawlist2oov_1(raws):  # raws - two-layer list
    ids = []  # store vocab ids and oov ids
    oovs = []  # store oov word in the src
    for r in raws:
        r_ids = []
        for w in r:
            if w in config.w2id:
                ...
                if w not in oovs:  # Add to list of OOVs
                    oovs.append(w)
                oov_num = oovs.index(w)  # This is 0 for the first article OOV, 1 for the second article OOV...
                r_ids.append(config.vocab_size + oov_num)
        ids.append(r_ids)

    return ids, oovs


def rawlist2oov(raws):  # raws - two-layer list; return flat
    ids = []  # store vocab ids and oov ids
    oovs = []  # store oov word in the src
    for r in raws:
        for w in r:
            if w in config.w2id:
                ...
                if w not in oovs:  # Add to list of OOVs
                    oovs.append(w)
                oov_num = oovs.index(w)  # This is 0 for the first article OOV, 1 for the second article OOV...
                ids.append(config.vocab_size + oov_num)
    return ids, oovs


def rawflat2oov(raws):
    ids = []  # store vocab ids and oov ids
    oovs = []  # store oov word in the src
    for w in raws:
        if w in config.w2id:
...
def flat_src(ids, raws, dis_scores, num_scores):
    flat_ids = []
    flat_raws = []
    flat_dis_scores = []
    flat_num_scores = []
    for i, id in enumerate(ids):
        flat_ids += id
        flat_raws += raws[i]
        flat_dis_scores += [dis_scores[i]] * len(id)
        flat_num_scores += [num_scores[i]] * len(id)
    return flat_ids, flat_raws, flat_dis_scores, flat_num_scores
