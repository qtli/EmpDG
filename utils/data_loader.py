...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # process input utterances, concepts, vad, vads todo: preprocess!
    ...
    def preprocess(self, arr, anw=False):
        ...
        if anw:
            ...
            X_ext, X_e_ext, X_oovs = self.process_oov(context, emotion_context)  # todo: only id for semantic context ?
            X_dial_ext += X_ext
...
def collate_fn(batch_data):
    ...
    mask_emotion_context, _ = merge(item_info['emotion_context_mask'])  # (bsz, max_context_len) dialogue state
...
class adver_Dataset(data.Dataset):
    """Custom data.Dataset compatible with data.DataLoader."""
    ...
    def __getitem__(self, index):
        """Returns one data pair (source and target)."""
        # {'context':[],'emotion_context':[],'target':[],'target_emotion':[],'feedback':[],'feedback_emotion':[],'emotion':[],'situation':[],'prediction':[],'pred_emotion':[]}
        item = {}
        item["context_text"] = self.data["context"][index]
        item["emotion_context_text"] = self.data["emotion_context"][index]
        item["target_text"] = self.data["target"][index]
...
def adver_collate_fn(batch_data):
    '''
    :param batch_data: dict
    :return:
    '''
...
def prepare_data_seq(batch_size=32, adver_train=False, train_d=False):
    pairs_tra, pairs_val, pairs_tst, vocab = load_dataset()  # read data from local
    logging.info("Vocab  {} ".format(vocab.n_words))

    # change
    if adver_train:
        dataset_train = adver_Dataset(pairs_tra, vocab)
        data_loader_tra = torch.utils.data.DataLoader(dataset=dataset_train,
                                                      batch_size=batch_size,
                                                      shuffle=True, collate_fn=adver_collate_fn)

        dataset_valid = adver_Dataset(pairs_val, vocab)
        data_loader_val = torch.utils.data.DataLoader(dataset=dataset_valid,
                                                      batch_size=batch_size,
                                                      shuffle=True, collate_fn=adver_collate_fn)

        dataset_test = adver_Dataset(pairs_tst, vocab)
        data_loader_tst = torch.utils.data.DataLoader(dataset=dataset_test,
                                                      batch_size=batch_size,
                                                      shuffle=False, collate_fn=adver_collate_fn)
    elif adver_train and train_d:
        dataset_train = d_Dataset(pairs_tra, vocab)
        data_loader_tra = torch.utils.data.DataLoader(dataset=dataset_train,
                                                      batch_size=batch_size,
                                                      shuffle=True, collate_fn=d_collate_fn)

        dataset_valid = d_Dataset(pairs_val, vocab)
        data_loader_val = torch.utils.data.DataLoader(dataset=dataset_valid,
                                                      batch_size=batch_size,
                                                      shuffle=True, collate_fn=d_collate_fn)

        dataset_test = d_Dataset(pairs_tst, vocab)
        data_loader_tst = torch.utils.data.DataLoader(dataset=dataset_test,
                                                      batch_size=1,
                                                      shuffle=False, collate_fn=d_collate_fn)
    else:
